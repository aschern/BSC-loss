...
class STSDataReaderBinaryPositives:
    """
    ...
    def __init__(self, dataset_folder, s1_col_idx=0, s2_col_idx=1, score_col_idx=2, delimiter="\t",
                 quoting=csv.QUOTE_NONE, normalize_scores=True, min_score=0, max_score=5, thr=0.5, get_positives=True):
        ...
        with gzip.open(filepath, 'rt', encoding='utf8') if filename.endswith('.gz') else open(filepath, encoding="utf-8") as fIn:
            ...
            for id, row in enumerate(data):
                try:
                    ...
                    if not self.get_positives or score > self.thr:
                        examples.append(InputExample(guid=filename+str(id), texts=[s1, s2], label=float(score > self.thr)))
                except:
                    continue
...
class STSBenchmarkDataReaderBinary(STSDataReaderBinaryPositives):
    """
